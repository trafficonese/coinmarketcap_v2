...
plot_top_currencies <- function(currency = "USD", k = 5, bar_color = "grey") {
    ...
    if (k > nrow(temp))
        warning(paste0("The argument provided to k is greater than the ",
                       "number of cryptocurrencies. Only ", nrow(temp),
                       " will be plotted."))
    ...
    ggplot(temp, aes_string("name", tolower(paste0("price_", currency)))) +
        ...
        ggtitle(paste0("Top ", k,
                       " Cryptocurrencies with Largest Marketcaps")) +
        coord_flip()
}
